...
// ==UserScript==
// @name         FreeBitCo.in auto free roll + play after
...
var counter = 0;
var lose_counter = 0;
const looseCountTrigger = 200;
const maxLooseCount = 250;
                    ...
                    gameLoop: function () {
                        ...
                        $('#double_your_btc_bet_lose').bind("DOMSubtreeModified", function (event) { // DOMSubtreeModified deprecated
                            if ($(event.currentTarget).is(':contains("lose")')) {
                                ...
                                ref.total_loses++;
                                lose_counter++;
                                if (lose_counter == looseCountTrigger) {
                                    var current = ref.__objects.betAmount.val();
                                    var multiply = (current * 2).toFixed(8);
                                    console.log('Loose %d times, so change multiply factor to %d', lose_counter, multiply);
                                    ref.__objects.betAmount.val(multiply);

                                    //setTimeout(function(){ location.reload(); }, 5); //Refresh page after win
                                }

                                if (lose_counter == maxLooseCount) {
                                    console.log('Loose %d times, so stop the game ...', lose_counter);
                                    ref.stopGame();
                                }
                        ...
                        $('#double_your_btc_bet_win').bind("DOMSubtreeModified", function (event) {
                            if ($(event.currentTarget).is(':contains("win")')) {
                                ...
                                if (counter == 2) {
                                    // setTimeout(function(){ location.reload(); }, 5); //Refresh page after win
                                    console.log('Won 2 times stopping the games ... ');
                                    ref.stopGame();
                                }
        ...
        Object.defineProperties(this, {
            ...
            startGame: {
                value: function () {
                    ...
                    this.resolveSite();
                    this.addChart();
                    console.log('Starting balance is : %d', this.convertToSatoshi(parseFloat(this.__objects.balance.text())));
    ...
    $(document).ready(function(){
        setInterval(function(){
            ...
            if ($('#play_without_captchas_button').is(':visible')) {
                ...
                console.log('Playing without captchas ...');
            }

            if ($('#free_play_form_button').is(':visible')) {
                $('#free_play_form_button').trigger('click');
                console.log('Free playing click...');
            }
        },5000);
    });
})();
